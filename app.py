#!/usr/bin/env python3
"""
麦田网站 — Flask 版（适合 PythonAnywhere / Render.com 部署）
前后端一体：同时提供静态页面 + API 接口
"""
...
@app.route("/maitian-app.js")
def maitian_js():
    return send_file(BASE_DIR / "maitian-app.js")

@app.route("/maitian-feed-api.js")
def maitian_feed_js():
    return send_file(BASE_DIR / "maitian-feed-api.js")

@app.route("/images-web/<path:filename>")
def images(filename):
    return send_file(BASE_DIR / "images-web" / filename)

@app.route("/uploads/<path:filename>")
def uploads(filename):
    return send_file(UPLOAD_DIR / filename.split("/")[-1])
...
def api_users():
    ...
    return jsonify([{**dict(zip(cols, r)), "password": "***"} for r in rows])
...
def api_login():
    ...
    if u["password"] != pw_hash(pwd): return jsonify({"ok":False,"msg":"密码错误"}), 401
    u["password"] = "***"
...
# --- 烦恼博物馆 ---
@app.route("/api/museum", methods=["GET", "POST"])
def api_museum():
    if request.method == "GET":
